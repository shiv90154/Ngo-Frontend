// app/(healthcare)/healthcare/page.tsx
"use client";
...
export default function HealthcareDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats({
        upcomingAppointments: appointmentsRes.data.total || 0,
        prescriptions: prescriptionsRes.data.total || 0,
        healthRecords: recordsRes.data.total || 0,
      });
    } catch (error) {
      console.error("Failed to load healthcare dashboard:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const quickActions = [
    { label: "Find Doctor", icon: Search, href: "/doctors", color: "from-blue-600 to-blue-700" },
    { label: "Book Appointment", icon: Calendar, href: "/doctors", color: "from-green-600 to-green-700" },
    { label: "Upload Record", icon: FileText, href: "/records", color: "from-purple-600 to-purple-700" },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/appointments" className="text-sm text-[#1a237e] hover:underline">
            See all
          </Link>
        </div>
        {upcomingAppointments.length === 0 ? (
          <div className="text-center py-6 text-gray-500">
            ...
            <Link href="/doctors" className="text-[#1a237e] text-sm mt-2 inline-block">
              Book an appointment →
            </Link>
          </div>
        ) : (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/prescriptions" className="text-sm text-[#1a237e] hover:underline">
            See all
          </Link>
        </div>
