...
import { GraduationCap, Home, BookOpen, PenTool, Users, LogOut, User, Menu, X, ChevronDown } from "lucide-react";
    ...
    <>
      <header className="hidden lg:block sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm">
        <div className="px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${isActive(item.href) ? "bg-[#1a237e]/10 text-[#1a237e]" : "text-gray-600 hover:bg-gray-50"}`}>
          ...
          <div className="relative">
            {mounted && (
              <>
                <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 p-1.5">
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border py-1 z-50">
                    ...
                    <button onClick={logout} className="w-full text-left px-4 py-2 text-red-600 hover:bg-red-50 flex items-center gap-2"><LogOut size={16} /> Logout</button>
                  </div>
                )}
              </>
            )}
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50">
        <div className="flex justify-around py-2">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className={`flex flex-col items-center ${isActive(item.href) ? "text-[#1a237e]" : "text-gray-500"}`}>
              <item.icon size={22} />
