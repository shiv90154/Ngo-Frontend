    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 flex items-center justify-center p-4">
            <div className="w-full max-w-5xl bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
                ...
                {/* Left Panel – Full version on tablet/desktop, condensed header on mobile */}
                <div className="md:w-1/2 bg-gradient-to-br from-blue-900 to-blue-800 text-white">
                    {/* Condensed version for mobile – only branding + one line */}
                    <div className="md:hidden p-4 flex items-center justify-between border-b border-blue-700">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <p className="text-[10px] opacity-80">Government of India</p>
                                <p className="font-bold text-sm">Samraddh Bharat</p>
                            </div>
                        </div>
                        <p className="text-xs italic text-blue-200">"Sabka Saath"</p>
                    </div>

                    {/* Full version for md+ screens */}
                    <div className="hidden md:flex md:flex-col md:justify-between md:h-full p-6 lg:p-8">
                        <div>
                ...
                <div className="w-full md:w-1/2 p-6 sm:p-7 md:p-8 bg-white">
                    <div className="max-w-sm mx-auto">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className={`w-full px-3 py-2.5 text-sm rounded-lg border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition ${error.email ? "border-red-400" : "border-gray-300"}`}
                                    />
                                    <FaEnvelope className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm" />
                                </div>
                                {error.email && <p className="text-xs text-red-500 mt-0.5">{error.email}</p>}
                            </div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className={`w-full px-3 py-2.5 text-sm rounded-lg border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition ${error.password ? "border-red-400" : "border-gray-300"}`}
                                    />
                                    <button
                                        type="button"
                                ...
                                {error.password && <p className="text-xs text-red-500 mt-0.5">{error.password}</p>}
                            </div>

                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-blue-700 hover:bg-blue-800 text-white font-medium py-2.5 rounded-lg transition shadow-md hover:shadow-lg disabled:opacity-50 text-sm"
                            >
                                {loading ? (
                        ...
                        <div className="mt-5 text-center space-y-1.5">
                            <Link href="/register" className="text-blue-700 hover:text-blue-800 text-xs md:text-sm font-medium inline-block">
                            ...
                            <div className="flex justify-between items-center text-xs">
                                ...
                                <Link href="/" className="text-gray-500 hover:text-blue-600 flex items-center gap-1">
                                    <FaArrowLeft className="text-[8px]" /> Back to Home
                                </Link>
                            </div>
                        </div>
